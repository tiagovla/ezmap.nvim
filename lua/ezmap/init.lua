...
local function map_(mappings, bufnr, default_opts)
    local fn
    if bufnr then
        fn = function(...) vim.api.nvim_buf_set_keymap(bufnr, ...) end
    else
    ...
    for _, m in pairs(mappings) do
        ...
        if (vim.tbl_islist(opts) and not vim.tbl_isempty(opts)) then
            local opts_ = {}
            for _, v in ipairs(opts) do opts_[v] = true end
            opts = opts_
        end
...
function M.map(mappings, default_opts) map_(mappings, nil, default_opts) end
